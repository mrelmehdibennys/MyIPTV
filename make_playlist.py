...
# هاد السطر كيفتح ملف EPG، ضروري يكون الملف موجود واخا خاوي
EPG_LIST = open('epglist.txt',"r") 
...
class Channel:
    def __init__(self, group, md_line):
        ...
        parts = md_line.split("|")
        # هنا كايقطع السطر باش يخرج المعلومات
...
def main():
    ...
    with open("playlist.m3u8", "w", encoding='utf-8') as playlist:
        # هنا كايجمع المعلومات
        processed_epg_list = ", ".join(EPG_LIST).replace('\n', '')
        head_playlist = f'#EXTM3U x-tvg-url="{processed_epg_list}"'
        print(head_playlist, file=playlist)
        
        # هنا كيدخل لمجلد lists باش يقرا الملفات
        os.chdir("lists")
